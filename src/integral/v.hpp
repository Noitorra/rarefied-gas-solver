...
namespace dod_vector {
    ...
    template<typename T>
    inline T dot(const V3<T>& u, const V3<T>& v) {
        return u[0] * v[0] + u[1] * v[1] + u[2] * v[2];
    }

    template<typename T>
    inline T sqr(const V3<T>& u) {
        return dot(u, u);
    }

    template<typename T>
    inline T norm(const V3<T>& u) {
        return std::sqrt(sqr(u));
    }

    template<typename T>
    inline V3<T> cross(const V3<T>& u, const V3<T>& v) {
    ...
    template<typename T>
    inline V3<T> rotate(const V3<T>& v, const V3<T>& n, double theta) {
    ...
    template<typename T>
    inline double angle(const T& u, const T& v) {
    ...
    inline const T1 dot(const V2<T1>& u, const V2<T2>& v) {
        return u[0] * v[0] + u[1] * v[1];
    }

    template<typename T>
    inline const T sqr(const V2<T>& u) {
        return dot(u, u);
    }

    template<typename T>
    inline const T norm(const V2<T>& u) {
        return std::sqrt(sqr(u));
    }

    template<typename T>
    inline const T arg(const V2<T>& v) {
        double n = norm(v);
        if (v[1] > 0.)
    ...
    template<typename T>
    inline const V2<T> rotate(const V2<T>& v, double theta) {
        double ct = std::cos(theta);
        double st = std::sin(theta);
